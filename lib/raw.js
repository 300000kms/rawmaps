...
! function (exports) {
	...
	raw.makeLegend = function (sc) {
		var html = '';
		for (var i in sc) {
			var c = chroma(sc[i]).css();
			html += "<span style='background:" + c + "!important; width:13px; height:10px; display:inline-block'></span>";
		}
		return html
	};
	...
	raw.models.set('timeSeries', function () {
		...
		stream.map(function (data) {
			...
			var results = d3.nest()
				...
				.rollup(function (v) {
					return {
						size: !size() ? v.length : d3.sum(v, function (e) {
							return size(e)
						}),
						date: date(v[0]),
						group: group(v[0]),
	...
	// Pointslite
	raw.models.set('pointslite', function () {
	...
	// Geometry 300mil
	raw.models.set('geometry', function () {

		var points = raw.model();

		var x = points.dimension('geometry')
	...
	raw.models.pointslite = raw.models.get('pointslite');
	raw.models.geometry = raw.models.get('geometry');
	...
	var chart_option = function () {
		...
		option.step = function (_) {
			if (!arguments.length) return step;
			option.step = _;
			return option;
		}

		option.min = function (_) {
			if (!arguments.length) return min;
			option.min = _;
			return option;
		}

		option.max = function (_) {
	...
	// Range option
	var chart_option_range = function () {
	...
	// List2 option
	var chart_option_list2 = function () {
	...
	raw.chart = function (id) {
		...
		chart.list2 = function (id) {
			var id = id || options.size();
			var option = chart_option_list2();
			options.set(id, option);
			return option;
		}
	...
	raw.map = function (id) {
		...
		chart.range = function (id) {
			var id = id || options.size();
			var option = chart_option_range();
			options.set(id, option);
			return option;
		}
...
//  new RegExp('^([A-z]{3,}(\\,)?\\s+)?(0?[1-9]|[12][0-9]|3[01])([A-z]{2})?[\\-\\_\\.\\/\\s]*(0?[1-9]|1[012])[\\-\\_\\.\\/\\s]*([0-9]{2,4})' + timeFormat + '$'),
	...
	raw.typeOf = function (value) {
		if (value === null || value === undefined || value.length === 0) return null; //bug
	...
	raw.divergingRange = function (n) {
		return d3.range(n).map(function (d) {
			//console.log(d3.hsl(360 / n * d, .82, .52).toString());
			return d3.hsl(360 / n * d, 0.5, 0.7).toString();
		})
	}
